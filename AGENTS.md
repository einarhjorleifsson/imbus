...
| `by_haul` | IMBUS D2.3 pre-computed catch totals per haul (Tier 2) | `dr_con("by_haul")` |
...
Generated by `dr_hl_length()` (`obus/R/dr_products.R`). One row per `.id × latin × length_cm`,
plus zero rows.
...
| `length_cm` | dbl | Standardised length in cm (`0` for zero rows) |
| `accuracy` | dbl | Length measurement resolution in cm (`NA` for zero rows) |
| `n_haul` | dbl | DataType-corrected numbers per haul (`0` for zero rows) |
| `n_hour` | dbl | DataType-corrected numbers per hour (`0` for zero rows) |
| `SpeciesValidity` | chr | Record type — **filter before analysis**; `NA` for zero rows |

**Derivation:** HL `inner_join`'d to valid-haul HH (`HaulValidity == "V"`); provides
`DataType` and `HaulDuration` for CPUE arithmetic. `LengthClass` NA rows and
`NumberAtLength == 0` rows removed. Sex and developmental stage collapsed (summed).
**Zero-filled** within each `Survey × Year × Quarter` — species absent from a haul
receive an explicit zero row. Presence-only records (both `NumberAtLength` and `TotalNumber`
are NA) excluded from zero-fill. Species names joined from `dr_con("species")` on `aphia`.

**No `SpeciesValidity` filter is applied** during product generation. `SpeciesValidity` is
retained as a grouping variable (interim design — pending clarification from ICES
Datacenter on correct handling). Filter to `SpeciesValidity == "1"` after collecting for
most analyses. 449 `.id × latin × length_cm` groups have more than one distinct
`SpeciesValidity` value — all 449 pair `"1"` with one other type: `{1, 5}` (410 groups)
or `{1, 10}` (39 groups).

### `by_haul` product schema

Generated by `dr_hl_haul()` (`obus/R/dr_products.R`). One row per `.id × latin`, plus zero
rows. The haul-level complement to `by_length`.
...
| `n_haul` | dbl | DataType-corrected numbers per haul (`0` for zero rows) |
| `n_hour` | dbl | DataType-corrected numbers per hour (`0` for zero rows) |
| `w_haul` | dbl | Total catch weight per haul in grams (`0` for zero rows; `NA` when weight not fully recorded) |
| `w_hour` | dbl | Total catch weight per hour in grams (`0` for zero rows; `NA` as for `w_haul`) |
| `SpeciesValidity` | chr | Record type — **filter before analysis**; `NA` for zero rows |

**Derivation:** Uses `TotalNumber` (not `NumberAtLength × SubsamplingFactor`) for counts
and `SpeciesCategoryWeight` for weights. Both are haul-level summary fields repeated across
all length rows within a `.id × aphia × sex × SpeciesCategory` group; deduplicated before
DataType-aware scaling. Sex and `SpeciesCategory` collapsed (summed). **Zero-filled** within
each `Survey × Year × Quarter`. Filter to `SpeciesValidity == "1"` after collecting.
...
**For catch totals per haul:** use `dr_con("by_haul")` — haul-level counts, numbers-per-hour,
and catch weights collapsed across length classes. Natural input for biomass-index and
